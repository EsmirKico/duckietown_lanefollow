...
try:
    from .jetson_optimization import apply_jetson_optimizations, cleanup_memory
except ImportError:
...
class DynamicController(DTROS):

    def __init__(self, node_name):
        # initialize the DTROS parent class
        super(DynamicController, self).__init__(node_name=node_name, node_type=NodeType.CONTROL)
        ...
        try:
            ...
            self.log(f"LED service not available: {e}")
        ...
        try:
            ...
            self.gain_calib = 1.0  # fallback value
    ...
    def overtake(self):
        ...
        self.log("Overtaking now, going left")
        for i in range(1, self.nr_steps+1): # changing d_offset as a discrete sine from 0 to pi/2
        ...
        while (t_start + self.leftlane_time) > rospy.get_rostime().secs:
            if self.head_state or self.duckie_left_state: #stop if vehicle or duckie are facing us on left lane
                self.logerr("Emergency stop while overtaking!")
                self.stop_prev = True
                self.stop = True
        ...
        self.log("Going back to the right")
        ...
        self.log("Overtaking done")
        self.overtaking = False

    def run(self):
        # Optimized rate for Jetson Nano - balance between responsiveness and CPU load
        rate = rospy.Rate(25)  # 25Hz instead of 50Hz for better performance on Jetson
        frame_count = 0
        ...
        while not rospy.is_shutdown():
            ...
            frame_count += 1
            if frame_count % 500 == 0:  # Cleanup every 500 iterations (~20 seconds)
                cleanup_memory()
                frame_count = 0
...
if __name__ == '__main__':
    # create the node
    node = DynamicController(node_name='dynamic_controller_node')
    # run node
    node.run()
    # keep spinning
    rospy.spin() 